...
struct Find_Result {
    ...
    // Not sure this version number makes any sense since at the time of writing:
    // - %VISUALSTUDIOVERSION% is "15.0",
    // - cl reports its version as 19.13.26128,
    // - vcvars reports itself as version 15.6.1, and 
    // - Visual Studio's install path contains the version number 14.13.26128. 
    // It is useful if want an environment variable to check if the script has been run yet though.
...
bool find_windows_kit_path(Version_Data *data, wchar_t *root, wchar_t *folder_name, Visit_Proc_W proc){
    auto folder = concat (root, folder_name);
    defer { free(folder); };

    visit_files_w (folder, data, proc);
...
void find_windows_kit_root(Find_Result *result) {
    ...
    if (windows10_root) {
        ...
        if(find_windows_kit_path(&data, windows10_root, L"Lib", win10_best))
        {
            result->windows_sdk_version = 10;
            result->windows_sdk_lib_root = data.best_name;
        }
        data = {0};
        if(find_windows_kit_path(&data, windows10_root, L"Include", win10_best))
        {
    ...
    // Look for a Windows 8 entry.
        ...
        if(find_windows_kit_path(&data, windows8_root, L"Lib", win8_best))
        {
...
bool find_visual_studio_2017_by_fighting_through_microsoft_craziness(Find_Result *result) {
    ...
    while (1) {
        ...
        auto msvc_path = concat(bstr_inst_path, L"\\VC\\Tools\\MSVC\\", version);
        auto library_path = concat(msvc_path, L"\\lib\\x64");
        ...
        if (os_file_exists(library_file)) {
            auto link_exe_path = concat(msvc_path, L"\\bin\\Hostx64\\x64");
            result->vs_version      = concat(L"", version);
            result->vs_exe_path     = link_exe_path;
            result->vs_library_path = library_path;
            result->vs_include_path = concat(msvc_path, L"\\include");
            return true;
        }
...
void find_visual_studio_by_fighting_through_microsoft_craziness(Find_Result *result) {
    ...
    for (int i = 0; i < NUM_VERSIONS; i++) {
        ...
        if (os_file_exists(vcruntime_filename)) {
            ...
            result->vs_include_path = concat(buffer, L"include");
            return;
        }
